...
<head>
<meta charset="utf-8">
<base href="file:///var/www/pdf-service/">
...
body { font-family: "Gilroy", sans-serif; color:#111; margin:0; }
...
.header .offer { color:#000; }
.header .bigger { font-size:13px; }
...
.num { width:40px; color:#9a9a9a; }
.sub { font-size:11px; color:#9a9a9a; margin-top:4px; }
.vol { width:110px; text-align:right; color:#000; }
.price { width:160px; text-align:right; font-weight:500; color:#000; }

/* ---------- TOTAL ---------- */
.total { margin-top:48px; font-size:30px; font-weight:700; text-align:right; }

/* ---------- INFO ---------- */
.info { display:flex; justify-content:space-between; margin-top:40px; font-size:12px; }
...
.footer .contacts { font-size:12px; text-align:right; }
</style>
</head>
...
app.post("/calculate", (req, res) => {
  try {
    ...
    res.json({ success: true, total: estimate.totalCash });
  } catch (e) {
...
app.post("/pdf", async (req, res) => {
  try {
    ...
    await page.setContent(buildHtml(req.body, estimate), { waitUntil: "networkidle" });
